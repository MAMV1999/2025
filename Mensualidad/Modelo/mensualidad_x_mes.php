...
class Mensualidadxmes
{
    ...
    public function listar()
    {
        $sql = "SELECT 
                    ...
                    CASE 
                        WHEN SUM(md.pagado) = COUNT(md.id) THEN 1
                        ELSE 0
                    END AS mes_cerrado
                FROM 
                    mensualidad_detalle md
                JOIN 
                    mensualidad_mes mm ON md.id_mensualidad_mes = mm.id AND mm.estado = 1
                JOIN 
                    institucion_lectivo il ON mm.id_institucion_lectivo = il.id
                WHERE 
                    md.estado = 1
                GROUP BY 
                    md.id_mensualidad_mes, mm.nombre, il.nombre
                ORDER BY 
                    md.id_mensualidad_mes;";
