...
class ReciboMatriculaTotal
{
    ...
    public function listarReciboMatriculaTotal()
    {
        $sql = "SELECT 
            ...
            JOIN 
                usuario_documento ud_alumno ON ua.id_documento = ud_alumno.id
            JOIN 
                usuario_apoderado uap ON md.id_usuario_apoderado = uap.id
            JOIN 
                usuario_documento ud_apoderado ON uap.id_documento = ud_apoderado.id
            JOIN 
                matricula_pago mp ON mp.id_matricula_detalle = md.id
            JOIN 
                matricula_metodo_pago mmp ON mp.id_matricula_metodo_pago = mmp.id
            WHERE 
                md.estado = 1 -- Mostrar solo registros activos
            ORDER BY 
                il.nombre ASC, -- Ordenar por lectivo
                iniv.nombre ASC, -- Ordenar por nivel
                ig.nombre ASC, -- Ordenar por grado
                isec.nombre ASC";
