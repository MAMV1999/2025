...
class ReporteMatricula
{
    ...
    public function matriculadospagos()
    {
        $sql = "SELECT 
                ...
                JOIN 
                    usuario_alumno ual ON md.id_usuario_alumno = ual.id
                JOIN 
                    matricula_pago mp ON md.id = mp.id_matricula_detalle
                JOIN 
                    matricula_metodo_pago mm ON mp.id_matricula_metodo_pago = mm.id
                WHERE 
                    md.estado = '1'
                ORDER BY 
                    mp.fecha ASC,
